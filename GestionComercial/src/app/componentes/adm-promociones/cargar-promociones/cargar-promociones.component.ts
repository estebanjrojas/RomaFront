...
export class CargarPromocionesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      this.promocionesForm.controls.id_promociones.setValue(params.promociones_id);
      console.log(params);
    });

    this.filteredOptions = this.promocionesForm.controls.producto.valueChanges
      .pipe(
        startWith<string | ProductosInterface>(''),
        map(value => typeof value === 'string' ? value : value.descrip),
        map(descrip => descrip ? this._filter(descrip) : this.productos.slice())
      );
  ...
  private _filter(descrip: string): ProductosInterface[] {
    ...
    return this.productos.filter(option => option.descrip.toLowerCase().indexOf(filterValue) === 0);
  }
  ...
  agregarDatosTablaRestricciones() {
    ...
    if (_tipo != null && _valor.length != 0 && _unidad != 0 && _unidad != undefined) {
