...
export class BuscarCategoriaComponent implements OnInit {
  ...
  buscarCategorias() {
    ...
    let descripcion = this.formBusquedaCategorias.get('buscar_descripcion').value;
    ...
    if (catpadre == true) { buscar_por_catpadre = 1; } else { buscar_por_catpadre = 0; }
    this.SrvCategorias.getCantidadPaginasCategorias(
      buscar_por_nombre, 
      buscar_por_descripcion, 
      buscar_por_catpadre, 
      txtBuscar).subscribe(respuesta => {
      console.log({ "SrvCategorias.getCantidadPaginasCategorias": respuesta });
