...
export class CargarCategoriaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      ...
      if (params.categorias_id != null) {
        ...
        this.categoriasForm.controls.id_categorias.setValue(params.categorias_id);
      }
  ...
  getDatosCategorias(categorias_id: any) {
    this.SrvCategorias.getDatosCategorias(categorias_id).subscribe(respuesta => {
      ...
      this.categoriasForm.controls.categorias_padre_id.setValue(cast[0].categorias_id_padre);
    }, err => {
  ...
  guardar() {
    ...
    const categoria: CategoriaGuardar = { "id": id_categoria, 'nombre': nombre, 'descripcion': descripcion, 'categorias_padre_id': categoria_padre };
