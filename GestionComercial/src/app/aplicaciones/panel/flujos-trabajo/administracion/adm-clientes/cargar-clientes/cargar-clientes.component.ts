...
@Component({
  ...
  styleUrls: ["./cargar-clientes.component.scss"],
})
export class CargarClientesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(
      ...
      (err) => {
        console.error(
          `Ocurrio un error al obtener el parametro del cliente. ${err}`
        );
      }
    );
    ...
    this.filteredOptions =
      this.clientesForm.controls.ciudades.valueChanges.pipe(
  ...
  getDatosCliente(id_clientes) {
    this.SrvClientes.getDatosClientePorId(id_clientes).subscribe(
      (respuesta) => {
        ...
        this.ciudades.push({
          id: cast[0].ciudades_id,
          descrip: cast[0].ciudad_nombre.trim(),
        });
      }
    );
  }
  ...
  obtenerProvincias() {
    ...
    this.SrvDomicilios.getProvinciasPorPais(1).subscribe(
      ...
      (error) => {
        console.error(
          `Ocurrio un error al obtener las provincias. ${error.message}`
        );
      },
      () => {
        ...
        this.SrvDomicilios.getCiudadesIdPorNombre(ciudad_nombre).subscribe(
          ...
          () => {
            this.SrvClientes.guardarClientePersonaDomicilio(
              ...
              this.snackBar.mostrarMensaje(
                "El cliente se ha cargado exitosamente"
              );
              this.clientesForm.reset();
              this.router.navigate(["clientes/busqueda-clientes"]);
            });
